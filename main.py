...
#usando a  lógica de não checar as cores das bolinhas       
def area_resgate():
    # anda até do outro lado com a garra abaixada
    abaixa_garra()
    motores.on_for_seconds(SpeedPercent(-50),SpeedPercent(-50),0.75)
    levanta_garra()
    motores.on_for_seconds(SpeedPercent(-10),SpeedPercent(-50),1) #vira esquerda 90°, isso quando chegou na parede
    motores.on_for_seconds(SpeedPercent(-50),SpeedPercent(-50),0.1) # anda só um pouco
    motores.on_for_seconds(SpeedPercent(-10),SpeedPercent(-50),1) #vira esquerda 90°, isso quando chegou na parede
    
    #Checa se os sensores estão vendo preto, pois indica a área de deixar as vítimas
    if(sensorEsq and sensorDir == preto):
        deixa_vitimas()
    ...
    motores.on_for_seconds(SpeedPercent(-50),SpeedPercent(-50),0.75)
    levanta_garra()
    motores.on_for_seconds(SpeedPercent(-10),SpeedPercent(-50),1) #vira direita 90°, isso quando chegou na parede
    motores.on_for_seconds(SpeedPercent(-50),SpeedPercent(-50),0.1) # anda só um pouco
    motores.on_for_seconds(SpeedPercent(-10),SpeedPercent(-50),1) #vira direita 90°, isso quando chegou na parede
...
def deixa_vitimas():
    motores.on_for_seconds(SpeedPercent(50),SpeedPercent(-50),0.75) #faz um giro de 180° pro armazenador ficar dentro da área preta
    funcao_estoque()
