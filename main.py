...
def vira_verde(valor_esq,valor_dir,verde):
    ...
    if valor_esq < verde and valor_dir < verde:
        pass
    elif (valor_esq < verde): #esq ve verde
...
def segue_linha(preto):
    ...
    if (valor_esq < verde) or (valor_dir < verde - 6): #sensor esquerdo ou direito vê o verde
        ...
        Ultimos_Esq = sensorEsq_valores[-90:]
        Ultimos_Esq.sort()
        Ultimos_Dir = sensorDir_valores[-90:]
        Ultimos_Dir.sort()
        if len(Ultimos_Esq) >= 90 and Ultimos_Esq[44] <= (preto - 5): #se o esquerdo andou vendo muito preto -> vira pra direita (virada bruta)
            motores.on(SpeedPercent(vmaior),SpeedPercent(vmenor))
            time.sleep(0.4)
        elif len(Ultimos_Dir) >= 90 and Ultimos_Dir[44] <= (preto + 5): #se o direito andou vendo muito preto -> vira pra esquerda (virada bruta)
...
def leitor_cor():
    print("SensorEsquerdo = ", sensorEsq.value()) #valores esq: branco (59) // preto (17) // verde (4 - 9)
    print("SensorDireito = ", sensorDir.value()) # valores dir: branco (70) // preto (14) // verde (4) // verde é menor que 6
